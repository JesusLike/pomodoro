...
from src.exceptions.data import TaskUnexistingCategoryError
...
class TasksDatabaseRepository():
    ...
    def create_task(self, props: dict[str, any]) -> DbTask:
        ...
        try:
            ...
            raise TaskUnexistingCategoryError() from e
        return db_task

    def get_task_with_user_id(self, id: int, user_id: int) -> DbTask | None:
        query = select(DbTask).where(DbTask.id == id, DbTask.user_id == user_id)
